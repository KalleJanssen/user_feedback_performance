...
df_long = pd.read_sql("SELECT * FROM ss_long_final_en_reconciled", database)
df_all = pd.read_sql("SELECT * FROM ss_all_final_en_reconciled", database)

df_all = df_all.append(df_long)

DBcursor.execute("DROP TABLE IF EXISTS combined_similarity_english")
DBcursor.execute("CREATE TABLE combined_similarity_english (text1 TEXT, preprocessed_text1 TEXT, text1_no_stop TEXT, text2 TEXT, preprocessed_text2 TEXT, text2_no_stop TEXT, similarityScore TINYINT(1))")
...
for index, row in df_all.iterrows():
    try:
        ...
        if len(no_stop_words1) > 2 and len(no_stop_words2) > 2:
            ...
            add_tweet = ("INSERT INTO combined_similarity_english(text1, preprocessed_text1, text1_no_stop, text2, preprocessed_text2, text2_no_stop, similarityScore)"
