...
for i in range(25):
    ...
    for index, row in df.iterrows():
        predicted_list.append(TextBlob(row[3]).sentiment.polarity)
        correct_list.append(row[1])

end = datetime.now().strftime("%d/%m/%Y %H:%M:%S.%f")
file.write("sentiment, textblob, " + start[:-3] + ", " + end[:-3] + "\n")
...
if not os.path.exists(filename):
    ...
    file.write("sentiment, textblob, " + str(correct/(correct+incorrect)) + ", " + str(precision) + ", " + str(recall) + ", " + str(F1) + ", " + str(MAE/len(predicted_list)) + "\n")
else:
    file = open(filename,"a")
    file.write("sentiment, textblob, " + str(correct/(correct+incorrect)) + ", " + str(precision) + ", " + str(recall) + ", " + str(F1) + ", " + str(MAE/len(predicted_list)) + "\n")
