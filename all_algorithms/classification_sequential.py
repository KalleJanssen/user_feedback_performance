...
for i in range(1):
    ...
    input_dim = X[:int(0.5*len(corpus))].shape[1]
...
file.write("classification, sequential, " + start[:-3] + ", " + end[:-3] + "\n")
file.close()
...
if not os.path.exists(filename):
    ...
    file.write("classification, sequential, " + str(correct/(correct+incorrect)) + ", " + str(precision) + ", " + str(recall) + ", " + str(F1) + "\n")
else:
    file = open(filename,"a")
    file.write("classification, sequential, " + str(correct/(correct+incorrect)) + ", " + str(precision) + ", " + str(recall) + ", " + str(F1) + "\n")
