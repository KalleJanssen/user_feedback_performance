...
def create_training(sql):
    ...
    y_train = df['class'].map({'ot': 0, 'br': 1, 'fr': 2})
...
file.write("classification_three, multinomial, " + start[:-3] + ", " + end[:-3] + "\n")
file.close()
...
if not os.path.exists(filename):
    ...
    file.write("classification_three, multinomial, " + str(correct/(correct+incorrect)) + ", " + str(precision) + ", " + str(recall) + ", " + str(F1) + ", 0\n")
else:
    file = open(filename,"a")
    file.write("classification_three, multinomial, " + str(correct/(correct+incorrect)) + ", " + str(precision) + ", " + str(recall) + ", " + str(F1) + ", 0\n")
